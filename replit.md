...
**November 22, 2025**:
*   **OpenAI Text-to-Speech Bible Audio**: Implemented Bible audio narration using OpenAI TTS API (paid service). Route `/api/bible/audio/:version/:book/:chapter` generates MP3 audio on-demand using `tts-1` model with `alloy` voice. Frontend includes simple play/stop controls in BibleReader header. No caching - audio is generated fresh for each chapter. Replaces removed free Bible Brain API integration.
